...
http://192.168.xxx.xxx?framesize=size     //size= UXGA|SXGA|XGA|SVGA|VGA|CIF|QVGA|HQVGA|QQVGA 
...
http://192.168.xxx.xxx/?SendCapturedImage    //Google Drive Line Notify
...
#include "Base64.h"              //base64 google
#include "soc/soc.h"             
...
//Command disassembly status value 
byte ReceiveState=0,cmdState=1,strState=1,questionstate=0,equalstate=0,semicolonstate=0;

// WARNING!!! Make sure that you have either selected ESP32 Wrover Module,
//            or another board which has PSRAM enabled

//Ancred ESP32-CAM module pin settings 
...
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);  //The setting of restarting after turning off the power is unstable 
  ...
  sensor_t * s = esp_camera_sensor_get();
  s->set_framesize(s, FRAMESIZE_QVGA);  //UXGA|SXGA|XGA|SVGA|VGA|CIF|QVGA|HQVGA|QQVGA  Set the initial image resolution 
  ...
  //Specify client static IP 
  //WiFi.config(IPAddress(192, 168, 201, 100), IPAddress(192, 168, 201, 2), IPAddress(255, 255, 255, 0));

  WiFi.begin(ssid, password);    //Perform network connection 
  ...
  {
      delay(500);
      if ((StartTime+10000) < millis()) break;    //Wait 10 seconds to connect 
  } 
  ...
  if (WiFi.status() == WL_CONNECTED) {    //If the connection is successful 
    ...
    for (int i=0;i<5;i++) {   //If connected to WIFI, set the flash to flash quickly 
      ledcWrite(4,10);
      delay(200);
      ledcWrite(4,0);
      delay(200);    
    ...
    for (int i=0;i<2;i++) {    //If you can't connect to WIFI, set the flash to flash slowly 
      ledcWrite(4,10);
      delay(1000);
      ledcWrite(4,0);
      delay(1000);    
...
//Customized homepage management interface 
  ...
  if (client) { 
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        getCommand(c);   //Disassemble the characters obtained in the buffer into command parameters 
                
        if (c == '\n') {
          if (currentLine.length() == 0) {    
            
            if (cmd=="getstill") {
              //Return JPEG format image 
              camera_fb_t * fb = NULL;
              ...
              //Return HTML homepage or Feedback 
              client.println("HTTP/1.1 200 OK");
              client.println("Access-Control-Allow-Headers: Origin, X-Requested-With, Content-Type, Accept");
              client.println("Access-Control-Allow-Methods: GET,POST,PUT,DELETE,OPTIONS");
        ...
        if ((currentLine.indexOf("/?")!=-1)&&(currentLine.indexOf(" HTTP")!=-1)) {
          if (Command.indexOf("stop")!=-1) {  //If the command contains the keyword stop, it will disconnect immediately  -> http://192.168.xxx.xxx/?cmd=aaa;bbb;ccc;stop
            client.println();
            client.println();
            client.stop();
          }
